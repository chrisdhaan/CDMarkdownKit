...
|---------|------------|----------------|
| 5.0.0   | 2026-08-11 | **Breaking:** removed `CDMarkdownParser`'s deprecated synchronous `parse(_:)` overloads (deprecated since v3.2.0); only the `async` overloads remain. Callers must add `await` at each call site — the signature is otherwise unchanged |
