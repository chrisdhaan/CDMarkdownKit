...
- Added an `intrinsicContentSize` override to `CDMarkdownTextView` and `CDMarkdownNSTextView`, so a text view used directly under UIKit or AppKit Auto Layout — with scrolling disabled and no explicit height constraint — grows to fit its content. On `CDMarkdownTextView` this reports a measured height only when `isScrollEnabled` is `false`; the default scroll-enabled view keeps `UITextView`'s behavior, so existing callers are unaffected. Both views re-invalidate the intrinsic size when their attributed text is set and when their width changes.
